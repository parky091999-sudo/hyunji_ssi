...
출력 형식 (반드시 이 순서대로, 각 블록 사이에 반드시 빈 줄 1개):

[훅 — 스크롤을 멈추게 하는 강력한 첫 문장 1줄]

[본문 3~5줄 — 구체적 사용 경험, 특징, 상황을 풀어서 설명. 문장마다 줄바꿈.]
...
중요: 본문과 해시태그 사이에 반드시 빈 줄 1개 넣을 것.

본문 작성 규칙:
1. 첫 줄(훅)은 무조건 강하게. 다음 중 하나의 느낌으로:
   - 공감 후벼파기: "겨울만 되면 ○○ 때문에 스트레스인 사람 봐봐"
   - 발견의 놀라움: "이거 왜 이제 알았지 진짜"
   - 후회 자극: "진작 알았으면 돈 굳었을 텐데"
   - 호기심: "이거 본 사람들 다 장바구니行 하더라"
...
4. 사회적 증거를 자연스럽게 녹여: "리뷰 수천 개", "재구매율 높은", "품절됐다 재입고된" 등
   (정확한 수치 모르면 "리뷰 좋은", "후기 많은" 정도로만)
5. ✔ 포인트는 실제 사용 상황을 구체적으로 (언제/어디서/어떻게 쓰는지)
6. 반말, 친근하게. 재치 있고 위트 있게. 과한 광고티 금지. 가격은 절대 언급하지 마.
7. 이모지는 본문에 1~2개까지만 자연스럽게. 해시태그 줄에는 이모지 넣지 마.
8. 글 전체가 500~700자 정도 되게 써줘. 너무 짧으면 안 돼.
...
★★★ 반드시 한국어로만 작성. 영어·중국어·일본어·태국어·베트남어·아랍어 등 다른 언어 절대 사용 금지.
