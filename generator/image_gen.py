"""
상품 이미지를 Gemini(gemini-3.1-flash-image)로 보조 컷 1장 생성 → imgBB 업로드
  - 메인은 원본 image_url(쿠팡/네이버 상세) 사용 — 호출부에서 첫 자리 유지
  - 이 모듈은 보조 컷 1장만: 3/4 비스듬 각도 + 옅은 그라데이션 배경
실패 시 [] 반환 → 호출부에서 원본만으로 단일 게시

설계 메모:
  - 원본 메인 사진이 직관적이고 깔끔하다는 운영자 피드백(2026-06-12, [017]) 반영.
  - 라이프스타일/사용 컷은 외형 왜곡 소지가 있어 제외.
"""
import base64
import io
import logging
