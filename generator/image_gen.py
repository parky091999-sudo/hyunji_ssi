...
상품 이미지를 Gemini(gemini-3.1-flash-image)로 클리닝 → imgBB 업로드
  - 1장: 정면 흰배경 클린샷 (텍스트/치수 제거)
  - 2장: 같은 제품 3/4 비스듬 각도(입체감) + 옅은 그라데이션 배경
실패 시 부분 결과(또는 [])로 반환 → 호출부에서 원본 이미지 유지

설계 메모:
  - 라이프스타일/사용 컷은 원본 외형 왜곡 소지가 있어 제외.
  - 두 컷 모두 동일 톤(흰/연회색)으로 유지해서 carousel 일관성 확보.
"""
import base64
import io
import logging
...
_PROMPTS = [
    ...
    (
        "같은 제품을 약 30~45도 비스듬한 각도(3/4 뷰)에서 본 전문 제품 사진으로 만들어줘. "
        "입체감과 깊이가 드러나는 구도 — 정면 컷과는 시점이 명확히 달라야 함. "
        "다음은 반드시 완전히 제거해야 해 — 한 글자도 남기지 마: "
